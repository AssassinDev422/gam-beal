...
class RegistrationView(utils.SendEmailViewMixin, generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # Send Welcome Email
        subject, from_email= 'Welcome to Gambeal!', 'support@gambeal.com', 
        html_content = render_to_string('welcome_gambeal.html', {}) # render with dynamic value
        text_content = strip_tags(html_content)
        msg = EmailMultiAlternatives(subject, text_content, from_email, [instance.email])
        msg.attach_alternative(html_content, "text/html")
        if instance.email == 'solodkiyruslan9@yandex.ru' or instance.email == 'qoshibotu@gmail.com':
            msg.send()
...
class UserView(generics.RetrieveUpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        if not request.user.is_prized and 'is_completed' in request.data and request.data['is_completed']:
            request.user.is_prized = True
            request.user.available_money += 0.5 #give prize $.50
