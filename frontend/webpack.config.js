...
module.exports = function(mode) {
    ...
    var config = {
        ...
        module: {
            loaders: [
                ...
                { test: /\.(otf)(\?v=\d+\.\d+\.\d+)?$/, loader: "url-loader?limit=10000" }
            ]
        },
        watchOptions: {}
    };
    ...
    if (isHot) {
        ...
        config.entry.unshift('webpack-dev-server/client?https://localhost:' + port, 'webpack/hot/only-dev-server');
        config.watch = true;
        config.watchOptions.poll = true;
        config.publicPath = 'https://localhost:' + port + '/static/';
        config.output.publicPath = 'https://localhost:' + port + '/static/';
